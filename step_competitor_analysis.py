...
# Any standalone bold line (a full line that's just "**Something**") inside
# a section body is treated as the start of unrequested content and cuts
# the body off there \u2014 this catches ANY extra section the model invents
# (not just specific known names like "Strategic White Spaces"), since it
# doesn't rely on a fixed disallow-list that could miss something new.
...
def _truncate_at_unexpected_heading(body: str, allowed_labels: frozenset = frozenset()) -> str:
    """
    Cuts a section's body off at the first standalone bold line that isn't
    one of `allowed_labels` (case-insensitive). For Competitive Landscape,
    allowed_labels is empty \u2014 no standalone bold line is expected there at
    all, so any one found is unrequested content. For SWOT Analysis, the 4
    sub-labels are allowed; anything else standalone-bold (e.g. a leaked
    "**Strategic White Spaces**") triggers the cut.
    """
